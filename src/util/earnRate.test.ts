...
describe('computeTotalEarnRate', () => {
  ...
  it('count multiplies the species rate', () => {
    const tanks = {
      'tide-pool': { fishCounts: { goldfish: 5 }, decorations: [] },
    };
  ...
  it('sums across multiple species in one biome', () => {
    const tanks = {
      'tide-pool': { fishCounts: { goldfish: 2, guppy: 3 }, decorations: [] },
    };
  ...
  it('sums across multiple biomes', () => {
    const tanks = {
      'tide-pool': { fishCounts: { goldfish: 1 }, decorations: [] },
      'open-reef': { fishCounts: { 'purple-tang': 2 }, decorations: [] },
      'abyss': { fishCounts: { anglerfish: 1 }, decorations: [] },
    };
    const expected =
  ...
  it('ignores unknown speciesIds (returns 0 for them)', () => {
    const tanks = {
      'tide-pool': { fishCounts: { 'mystery-fish': 10 }, decorations: [] },
    };
    expect(computeTotalEarnRate(tanks)).toBe(0);
  });
});
